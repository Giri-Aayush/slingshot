...
- **Screenshot** — `/usr/sbin/screencapture` grabs the full desktop to `~/Pictures/AirGrab/`.
...
1. Download `AirGrab.zip` from [Releases](https://github.com/Giri-Aayush/slingshot/releases) and unzip.
2. The app is signed with a development certificate, not notarized, so Gatekeeper will balk. Either:
   - run `xattr -dr com.apple.quarantine /path/to/AirGrab.app`, or
...
4. Grant **Screen Recording** (System Settings → Privacy & Security → Screen & System Audio Recording → enable AirGrab), then quit and reopen the app — that permission only applies at launch.
...
mkdir -p AirGrab.app/Contents/MacOS
cp Info.plist AirGrab.app/Contents/Info.plist
cp .build/release/AirGrab AirGrab.app/Contents/MacOS/AirGrab
codesign --force -s "<your signing identity>" AirGrab.app
open AirGrab.app
```

Requires macOS 13+, Swift 5.9+. Sign with a real certificate (`security find-identity -v -p codesigning`), **not** ad-hoc (`-s -`): TCC ties Screen Recording grants to the code signature, and an ad-hoc signature changes on every rebuild — the Settings toggle will show "on" while silently failing. If you hit that state, `tccutil reset ScreenCapture com.aayush.airgrab` and re-grant.

## Troubleshooting

| Symptom | Fix |
| --- | --- |
| "AirGrab Not Opened / could not verify" | Strip quarantine: `xattr -dr com.apple.quarantine AirGrab.app` |
| Screenshot toggle on, but captures fail | Stale TCC grant from a re-signed build: `tccutil reset ScreenCapture com.aayush.airgrab`, reopen, re-grant |
...
| Peers never connect | Same Wi-Fi? Firewall blocking incoming connections? Check `~/Library/Logs/AirGrab.log` on both sides |
...
Everything the app does is narrated in `~/Library/Logs/AirGrab.log` (also reachable from the menu bar → Show Log).
